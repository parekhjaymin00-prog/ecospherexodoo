...
export default function CustomReportBuilder() {
  const [filters, setFilters] = useState({ module: '', startDate: '', endDate: '', status: '', groupBy: 'month' });
          ...
          {data.data.length === 0 ? <EmptyState title="No data matches your filters" /> : (
            <div className="bg-[#171717] border border-[#2A2A2A] rounded-lg p-4 overflow-x-auto">
              <table className="w-full text-sm">
                <thead><tr className="text-left text-[#737373] border-b border-[#2A2A2A]"><th className="pb-3 font-medium">Module</th><th className="pb-3 font-medium">Type</th><th className="pb-3 font-medium">Date</th><th className="pb-3 font-medium">Description</th><th className="pb-3 font-medium">Value</th><th className="pb-3 font-medium hidden md:table-cell">Dept</th></tr></thead>
                <tbody>
                  {data.data.slice(0, 50).map((d, i) => (
                    <tr key={i} className="border-b border-[#2A2A2A] hover:bg-[#1F1F1F]">
                      <td className="py-2 text-[#A3A3A3]">{d.module}</td>
                      <td className="py-2 text-white">{d.type}</td>
                      <td className="py-2 text-[#A3A3A3]">{new Date(d.date).toLocaleDateString()}</td>
                      <td className="py-2 text-white max-w-[200px] truncate">{d.description}</td>
                      <td className="py-2 text-white">{d.value} {d.unit}</td>
                      <td className="py-2 text-[#A3A3A3] hidden md:table-cell">{d.department}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
              {data.total > 50 && <p className="text-xs text-[#525252] text-center mt-3">Showing 50 of {data.total} records. Export for full data.</p>}
            </div>
          )}
        </div>
      )}
