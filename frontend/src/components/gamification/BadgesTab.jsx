...
export default function BadgesTab() {
  ...
  const fetchBadges = async () => {
    setLoading(true);
    try {
      const [badgeRes, myRes] = await Promise.all([api.get('/badges'), api.get('/badges/my')]);
      setBadges(badgeRes.data.badges);
      setEarnedIds(new Set(myRes.data.earnedBadgeIds));
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <p className="text-xs text-[#737373]">Earned: {earnedIds.size} / {badges.length}</p>
        <button onClick={openCreate} className="px-4 py-2 text-sm bg-white text-black font-medium rounded-lg hover:bg-[#D4D4D4]">Add Badge</button>
      </div>
      {loading ? <div className="animate-pulse grid grid-cols-2 md:grid-cols-4 gap-4">{Array.from({length:4}).map((_,i)=><div key={i} className="h-32 bg-[#2A2A2A] rounded-lg"/>)}</div> : badges.length === 0 ? <EmptyState title="No badges" message="Create your first badge" /> : (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {badges.map((b) => {
            ...
            return (
              <div key={b.id} className={`bg-[#171717] border rounded-lg p-4 text-center relative ${isEarned ? 'border-white' : 'border-[#2A2A2A] opacity-60'}`}>
                {isEarned && (
                ...
                {isEarned && <p className="text-[10px] text-green-400 mt-1 font-medium">Earned ✓</p>}
                {!isEarned && <p className="text-[10px] text-[#525252] mt-1">{b.unlockRule}</p>}
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Badge' : 'Create Badge'} size="md">
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div><label className="block text-xs text-[#A3A3A3] mb-1">Unlock Rule * <span className="text-[#525252]">(e.g. &quot;XP {'>'}= 100&quot; or &quot;5 challenges completed&quot;)</span></label><input value={form.unlockRule} onChange={(e) => setForm({...form, unlockRule: e.target.value})} className="w-full px-3 py-2 text-sm bg-[#0A0A0A] border border-[#2A2A2A] rounded-lg text-white focus:border-[#525252] focus:outline-none" /></div>
