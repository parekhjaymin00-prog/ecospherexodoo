...
export default function RewardsTab() {
  ...
  const fetchRewards = useCallback(async () => {
    setLoading(true);
    try {
      const [rwRes, ptRes] = await Promise.all([
        api.get('/rewards', { params: { page, limit: 12 } }),
        api.get('/rewards/my-points'),
      ]);
      setRewards(rwRes.data.rewards);
      setPagination(rwRes.data.pagination);
      setMyPoints(ptRes.data.totalPoints);
  ...
  const handleSave = async (e) => {
    e.preventDefault(); if (!form.name || !form.pointsRequired) return; setSaving(true);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="bg-[#171717] border border-[#2A2A2A] rounded-lg px-4 py-2">
          <p className="text-xs text-[#737373]">Your Points Balance</p>
          <p className="text-lg text-white font-bold">{myPoints} pts</p>
        </div>
        <button onClick={openCreate} className="px-4 py-2 text-sm bg-white text-black font-medium rounded-lg hover:bg-[#D4D4D4]">Add Reward</button>
      </div>

      {loading ? <div className="animate-pulse grid grid-cols-1 md:grid-cols-3 gap-4">{Array.from({length:3}).map((_,i)=><div key={i} className="h-48 bg-[#2A2A2A] rounded-lg"/>)}</div> : rewards.length === 0 ? <EmptyState title="No rewards" message="Create your first reward" /> : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {rewards.map((r) => {
            const canRedeem = r.stock > 0 && myPoints >= r.pointsRequired;
            const isOutOfStock = r.stock <= 0;
            const pointsNeeded = r.pointsRequired - myPoints;
            return (
              <div key={r.id} className="bg-[#171717] border border-[#2A2A2A] rounded-lg p-5">
                ...
                <button
                  ...
                  className={`w-full py-2 text-sm font-medium rounded-lg mb-2 transition-colors ${
                    canRedeem ? 'bg-white text-black hover:bg-[#D4D4D4]' : 'bg-[#2A2A2A] text-[#525252] cursor-not-allowed'
                  }`}
                >
                  {redeeming === r.id ? 'Redeeming...' : isOutOfStock ? 'Out of Stock' : !canRedeem ? `Need ${pointsNeeded} more pts` : 'Redeem'}
                </button>

                <div className="flex gap-2 justify-end">
                  <button onClick={() => openEdit(r)} className="text-[#A3A3A3] hover:text-white text-xs">Edit</button>
